...
namespace dataset
{
    namespace
    {
        ...
        inline void set_error(Error e) { g_last_error.store((int32_t)e, std::memory_order_relaxed); }
        ...
        {
            PngImg out{0, 0, {}};
            dataset::detail::mmap_ro m = dataset::detail::mmap_file_ro(path);
            if (!m.ptr)
            {
    ...
    int build_hostpack(const BuildConfig& cfg, const std::string& out_path)
    {
        ...
        for (size_t i = 0; i < N; i++)
        {
            ...
            size_t bytes = (size_t)row_stride * h;
        ...
        hdr.scene_off = hdr.scene_off;
        hdr.cam_off = hdr.cam_off;
        hdr.frames_off = hdr.frames_off;
        hdr.pixels_off = hdr.pixels_off;
        hdr.end_off = (uint64_t)cur;
        hdr.bytes_total = hdr.end_off;
        std::memcpy(&scene, &scene, sizeof(SceneRec));

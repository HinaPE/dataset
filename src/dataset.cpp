...
inline size_t rup(size_t x, size_t a) { return (x + (a - 1)) & ~(a - 1); }
...
Error last_error() { return (Error)g_last_error.load(std::memory_order_relaxed); }
int build_hostpack(const BuildConfig& cfg, const std::string& out_path) {
  ...
  threads.reserve(th);
  for (uint32_t t = 0; t < th; t++) threads.emplace_back([&] {
    for (;;) {
  ...
  for (size_t i = 0; i < N; i++) if (!imgs[i].w) {
    set_error(Error::IoFail);
    return -1;
  }
  ...
  hdr.caps_bits = 0;
  auto wr = [&](const void* p, size_t n) { return write_exact(fo, p, n); };
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    tt[i] = (uint32_t)i;
    for (int j = 0; j < 12; j++) T[i * 12 + j] = meta.items[i].T[j];
  }
  ...
  float lutf[256];
  if (cfg.pixel_format == PixelFormat::RGBA32F) srgb_lut(lutf);
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    if (cfg.pixel_format == PixelFormat::RGBA8) {
      ...
      for (int y = 0; y < h; y++) {
        ...
        for (int x = 0; x < w; x++) {
          unsigned char r = s[x * 4 + 0], g = s[x * 4 + 1], b = s[x * 4 + 2], a = s[x * 4 + 3];
          dstf[x * 4 + 0] = lutf[r];
          dstf[x * 4 + 1] = lutf[g];
          dstf[x * 4 + 2] = lutf[b];
...
size_t frame_count(PackHandle ph) { auto h = (PackHandleImpl*)ph; return h ? h->frames.size() : 0; }
size_t camera_count(PackHandle ph) { auto h = (PackHandleImpl*)ph; return h ? (size_t)h->cam.count : 0; }
size_t frame_camera_index(PackHandle ph, size_t i) { auto h = (PackHandleImpl*)ph; if (!h || i >= h->frames.size()) return 0; return (size_t)h->frames[i].camera_id; }
ImageView image_view(PackHandle ph, size_t i) { ImageView v{}; auto h = (PackHandleImpl*)ph; if (!h || i >= h->frames.size()) return v; const FrameRec& fr = h->frames[i]; v.data = (const void*)(h->base + fr.pixel_off); v.width = fr.width; v.height = fr.height; v.row_stride = fr.row_stride; v.pixel_stride = fr.pixel_stride; v.format = (PixelFormat)h->hdr.pixel_format; v.roi_x = fr.roi_x; v.roi_y = fr.roi_y; v.roi_w = fr.roi_w; v.roi_h = fr.roi_h; return v; }
CameraSOAView camera_soa(PackHandle ph) { CameraSOAView v{}; auto h = (PackHandleImpl*)ph; if (!h) return v; const char* base = h->base; const CamSOA& c = h->cam; v.fx = (const float*)(base + c.fx_off); v.fy = (const float*)(base + c.fy_off); v.cx = (const float*)(base + c.cx_off); v.cy = (const float*)(base + c.cy_off); v.T3x4 = (const float*)(base + c.T_off); v.width = (const uint32_t*)(base + c.w_off); v.height = (const uint32_t*)(base + c.h_off); v.time = (const uint32_t*)(base + c.time_off); v.count = (size_t)c.count; return v; }
void scene_aabb(PackHandle ph, float out_min[3], float out_max[3]) { auto h = (PackHandleImpl*)ph; if (!h) { out_min[0] = out_min[1] = out_min[2] = 0; out_max[0] = out_max[1] = out_max[2] = 0; return; } for (int i = 0; i < 3; i++) { out_min[i] = h->scene.aabb_min[i]; out_max[i] = h->scene.aabb_max[i]; } }
ColorSpace scene_color_space(PackHandle ph) { auto h = (PackHandleImpl*)ph; if (!h) return ColorSpace::Linear; return (ColorSpace)h->hdr.color_space; }
PixelFormat pack_pixel_format(PackHandle ph) { auto h = (PackHandleImpl*)ph; if (!h) return PixelFormat::RGBA8; return (PixelFormat)h->hdr.pixel_format; }
int hostpack_version(PackHandle ph) { auto h = (PackHandleImpl*)ph; if (!h) return 0; return (int)h->hdr.version; }
Caps pack_caps(PackHandle ph) { Caps c{}; auto h = (PackHandleImpl*)ph; if (!h) { c.bits = 0; return c; } c.bits = h->hdr.caps_bits; return c; }
uint64_t pack_bytes(PackHandle ph) { auto h = (PackHandleImpl*)ph; if (!h) return 0; return h->hdr.bytes_total; }
}
